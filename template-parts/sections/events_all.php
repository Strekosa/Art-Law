...
<section class="events-all">
    <?php $featured_posts = get_sub_field('events_all'); ?>
    <div class="container-boxed flex column ">
        <?php if ($field = get_sub_field('title')): ?>
            <div class="events-all-title flex justify-between <?php if(!$featured_posts){echo 'justify-center sm-justify-start';}?>">
                <h2 class="general"><?php echo $field; ?></h2>
                <?php if($featured_posts):?>
                <div class="arrows flex justify-end align-center">
                    <!-- Next page -->
        ...
        if ($featured_posts):
            ...
            ?>
            <div class="events-all-main events-all-slider">
                <?php foreach ($featured_posts as $post):
                    ...
                    if ($link):
                        ...
                        ?>
                    <a class="events-all-item  flex column align-start slick-slide" href="<?php echo esc_url($link_url); ?>"
                       target="<?php echo esc_attr($link_target); ?>">

                        <div class="events-all-item-img">
                            <?php the_post_thumbnail('full', array('class' => 'img-responsive')); ?>
                        </div>
                        <div class="events-all-item-content flex column">
                            <h3><?php the_title(); ?> </h3>
                            <div class="events-all-item-info flex column">
                                <div class="events-all-item-date">
                                    <?php echo date("l,F j, Y", strtotime($date)); ?>
                                    at <?php echo date("g:i a", strtotime($date)); ?>
                                </div>
                                <div class="events-all-item-subtitle">
            ...
            wp_reset_postdata(); ?>
        <?php else: ?>
            <p class="no-events <?php if(!$featured_posts){echo 'text-center';}?>">Check back soon for updates!</p>
        <?php endif; ?>
